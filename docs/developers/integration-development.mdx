...
<AiQuickStart
  ...
  docsUrl="https://docs.singularitybridge.net/developers/integration-development"
...
```
sb-api-services-v2/src/integrations/your_service/
├── integration.config.json     # Required: Metadata and API key definitions
├── your_service.actions.ts     # Required: Action definitions and handlers
├── your_service.service.ts     # Optional: Business logic and API calls
...
| `type` | `"secret"` \| `"text"` | Yes | `secret` = password field, `text` = visible input |
...
import { getCompanyApiKeyValue } from '../../services/api.key.service';

export async function performAction(
  companyId: string,
  params: ActionParams,
): Promise<ActionResult> {
  // Fetch API key from encrypted storage
  const apiKey = await getCompanyApiKeyValue(companyId, 'your_service_api_key');

  if (!apiKey) {
    throw new Error('Your Service API key is not configured');
  }

  // Make API call
  const response = await fetch('https://api.yourservice.com/v1/action', {
    method: 'POST',
    headers: {
